...
If by chance a MOC needs to be broken up into 2 different MOC's due to its size, then only the contituent notes need to have their `[MOC Links](MOC%20Links)` updated to point to the new broken-out MOC.
...
For example, if you create a new note about **Marcus Aurelius**, and in that note you include a link in the note to the `[](Philosophy%20MOC#Stoicism)` section, the note will automatically show up under the Stoicism section of your MOC, without you having to do anything else, no editing of the MOC is required, only a new note pointing to that section that the Dataview query is scanning for.
...
```dataview
list from ""
where contains(file.outlinks, [](Philosophy%20MOC#Stoicism))
...
```dataview
list from ""
where contains(file.outlinks, [](Philosophy%20MOC#Marcus%20Aurelius))
sort file.mtime desc
...
```dataview
list from ""
where contains(file.outlinks, [](Philosophy%20MOC#Existentialism))
sort file.mtime desc
...
- **Search**: Look for any notes that link to the `[](Philosophy%20MOC#Stoicism)` section.
...
```
#### **MapOfContent:** [](Philosophy%20MOC#Stoicism) - [](Optional#Second-MOC-Link) - [More MOC Links as needed](More%20MOC%20Links%20as%20needed)
---
Your note content goes here.

---
# References
- [Link to relevant sources](Link%20to%20relevant%20sources)

```

By including the link `[](Philosophy%20MOC#Stoicism)` in the `MapOfContent` field, you ensure that the note is categorized under the Stoicism section in your MOC.
...
```dataview
list from ""
where contains(file.outlinks, [](Coding%20MOC#Python))
sort file.mtime desc
...
When you promote a section into its own MOC, all the notes that previously linked to the old section in the **Technology MOC** (e.g., `[](Technology%20MOC#Coding)`) must now be updated to point to the appropriate section in the **Coding MOC**.
...
```
#### **MapOfContent:** [](Technology%20MOC#Coding)
...
```
#### **MapOfContent:** [](Coding%20MOC#Python)
...
4. **Modify the MOC Links** in each individual note to ensure they point to the correct MOC (e.g., `[](Coding%20MOC#Python)` instead of `[](Technology%20MOC#Coding)`).
...
In contrast, the **MOC + Dataview** system automates much of this overhead. With a single link to an MOC section (e.g., `[](Philosophy%20MOC#Stoicism)`), your notes are dynamically categorized and displayed. Once the Dataview queries are in place, **they automatically pull in and update notes**, saving you from having to manually organize your notes into folders or manually curate index pages. This eliminates the constant reorganization that comes with other methods, allowing you to **focus on content creation** rather than content management.
...
With MOCs, however, notes can easily belong to **multiple MOCs** without duplication. A note about **Marcus Aurelius - Meditations** can be linked to both `[](Philosophy%20MOC#Stoicism)` and `[](History%20MOC#Roman%20Empire)`, ensuring it appears under both subjects. This kind of **cross-linking** allows you to organize information according to its content and relevance to multiple subjects, without the limitations imposed by folders.
