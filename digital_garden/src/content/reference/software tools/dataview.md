...
Operates on metadata in your markdown files. Cannot read everything from the vault, only specific data from the [YAML frontmatter](reference/software%20tools/YAML.md) or from the [inline fields](https://blacksmithgu.github.io/obsidian-dataview/annotation/add-metadata/#inline-fields) via `[key::value]` syntax.
...
You can either access the properties of the current note via `this` object `this.file.name`, or access different note via `[link_to_note](link_to_note).file.name`. Switch to edit mode to view the syntax:

example: `=[_workflow](_workflow).file.name` is last modified `=[_workflow](_workflow).file.mtime`
...
- [dataview - index page](_dataview_database/index.md) - personal note
